...
class Trainer(object):
    def __init__(self, args):
        ...
        # --- MATCHER ---
        # Note: knn_path removed — semantic signal is already baked into symbol_emb
        # via the BERT-concat at load time. The gate in neighbor_encoder is for
        # future extension if you want runtime semantic neighbors.
        ...
        # Looser schedule: halve every 3000 steps instead of 1000.
        # With 5000 batches at step=1000, LR was essentially dead by batch 4000.
        # At step=3000 it halves once at 3000, still has useful LR through training.
    ...
    def load_embed(self):
        ...
        if getattr(self, 'use_semantic', False):
            sb_file = getattr(self, 'semantic_path',
                              os.path.join(os.path.dirname(self.dataset), f'{self.prefix}_anchors.npy'))
    ...
    def build_connection_matrix(self, max_=100):
        ...
        FIX: neighbors are shuffled before truncation to remove file-order bias.
        Previously, [:max_] always took the first N lines from path_graph,
        which caused high-degree entities (like those in FB15k) to have
        systematically biased neighbor sets missing entire relation types.
        """
        ...
        for ent, idx in self.ent2id.items():
            ...
            # --- KEY FIX: shuffle before truncating so we get a representative
            #     sample of relation types rather than whatever the file puts first.
    ...
    def train(self):
        ...
        for data in train_generate_medical(self.dataset, self.batch_size, self.train_few,
            ...
            rel_id = self.symbol2id.get(rel_name, self.pad_id)
            ...
            if self.batch_nums % self.log_every == 0:
                logging.critical(f"Batch {self.batch_nums}: Loss={np.mean(losses):.4f} "
