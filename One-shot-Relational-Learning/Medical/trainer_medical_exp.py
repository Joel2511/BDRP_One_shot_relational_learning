...
class Trainer(object):
    def __init__(self, arg):
        ...
        # Looser schedule: halve every 3000 steps.
        # Original step_size=1000 killed LR before training converged.
        self.scheduler = optim.lr_scheduler.StepLR(self.optim, step_size=3000, gamma=0.5)

        # FIX: was `args.use_semantic` which is not in scope here.
        # self.use_semantic is already set by the setattr loop above.
    ...
    def build_connection(self, max_=100):
        """
        Build neighbor connection matrix.
        FIX: shuffle neighbors before truncation to remove file-order bias.
        Medical ontology graphs have hub nodes with 60+ connections; without
        shuffling the [:max_] slice always takes the same relation types.
        """
        ...
        for ent, id_ in self.ent2id.items():
            ...
            # Shuffle before truncating so high-degree hub nodes get a
            # representative sample of relation types, not just file-order first N.
            ...
            for idx, (rid, eid) in enumerate(neighbors):
                self.connections[id_, idx, 0] = rid
                self.connections[id_, idx, 1] = eid
        return degrees
    ...
    def train(self):
        ...
        for rel, freq in relation_freq.items():
            ...
            if esc in self.symbol2id:
                ...
                w = float(min(max((max_freq / (freq + epsilon)) ** 0.5, 1.0), 10.0))
                weight_tensor[rid] = w
        ...
        for data in train_generate_medical(self.dataset, self.batch_size, self.train_few,
            ...
            # FIX: was passing query_meta for false triples — wrong neighborhood context for negatives.
            ...
            if self.no_meta:
                ...
                # FIX: now correctly uses false_meta instead of query_meta
                false_scores = self.matcher(false, support, false_meta, support_meta)

            # Adversarial negative weighting — safe version that handles uneven batch sizes
            adv_temperature = 1.0
            q_size = query_scores.size(0)
            f_size = false_scores.size(0)

            if f_size >= q_size and f_size % q_size == 0:
                neg_num = f_size // q_size
                f_scores = false_scores.view(q_size, neg_num)
                with torch.no_grad():
                    adv_weights = F.softmax(f_scores * adv_temperature, dim=-1)
                adv_false_scores = (adv_weights * f_scores).sum(dim=-1)
            else:
                # Fallback: plain mean over all negatives if shapes don't align
                adv_false_scores = false_scores.mean().expand(q_size)
            ...
            if self.batch_nums % self.log_every == 0:
                ...
                logging.critical(f"Batch {self.batch_nums}: Loss={avg_loss:.4f} "
                                 f"(Task: {rel_name}, Weight: {task_weight:.1f})")
            ...
            for triple in tqdm(test_tasks[query_][few:], desc=f"Tasks in {query_}", leave=False):
                ...
                for i in range(0, len(valid_candidate_data), EVAL_BATCH_SIZE):
                    ...
                    if meta:
                        ...
                        query_meta_ = tuple(t.to(self.device, non_blocking=True)
                                            for t in self.get_meta(batch_left, batch_right))
                        scores_t = self.matcher(query_batch, support, query_meta_, support_meta)
                    else:
