...
class EmbedMatcher(nn.Module):
    ...
    def forward(self, query, support, query_meta=None, support_meta=None):
        ...
        # *** UNIFIED META UNPACKING - WORKS FOR ALL DATASETS ***
        if len(query_meta) == 4:
            # NELL-One: full 4-tuple (l_conn, l_deg, r_conn, r_deg)
            q_l_conn, q_l_deg, q_r_conn, q_r_deg = query_meta
            s_l_conn, s_l_deg, s_r_conn, s_r_deg = support_meta
        else:
            # Medical (object_only=True): 2-tuple (l_conn, r_conn)
            q_l_conn, q_r_conn = query_meta
            s_l_conn, s_r_conn = support_meta
            q_l_deg = q_r_deg = torch.zeros_like(q_l_conn[:,:1]).long()
            s_l_deg = s_r_deg = torch.zeros_like(s_l_conn[:,:1]).long()
