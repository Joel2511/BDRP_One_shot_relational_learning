...
from matcher_two_hop import EmbedMatcher # Ensure this file exists and handles 2-hop inputs
...
class Trainer(object):
    ...
    def __init__(self, arg):
        ...
        if not torch.cuda.is_available():
            ...
            logging.warning("No CUDA found. Running on CPU (will be slow).")
        else:
        ...
        if self.test or self.random_embed:
            ...
            self.load_embed() # <--- Uses the robust loader defined below
        ...
        # ATOMIC is dense, so these steps might take a minute
        ...
        self.connections_2hop = torch.LongTensor(self.connections_2hop).to(self.device)
    ...
    def load_embed(self):
        ...
        # --- ATOMIC FIX: Robust loading for ComplEx ---
        if self.embed_model in ['DistMult', 'TransE', 'RESCAL', 'TransH']:
            ...
            try:
                # Try loading as complex and flattening
                ...
                if 'to float' in str(e) or 'complex' in str(e):
                    logging.warning(f"Complex loading failed. Assuming already flattened. Error: {e}")
    ...
    def build_connection(self, max_=100):
        ...
        with open(self.dataset + '/path_graph') as f:
            lines = f.readlines()
            for line in tqdm(lines, desc="Building 1-hop connections"):
    ...
    # --- NEW: 2-hop neighbor building ---
    def build_2hop_connection(self, max_=100):
        # Allocate a tensor for 2-hop neighbors with same shape as 1-hop connections
        self.connections_2hop = (np.ones((self.num_ents, max_, 2)) * self.pad_id).astype(int)

        # --- ATOMIC FIX: Added tqdm because this loop is heavy on dense graphs ---
        logging.info("Constructing 2-hop neighbors... this may take a while.")
        for ent, id_ in tqdm(self.ent2id.items(), desc="Building 2-hop connections"):
            ...
            if len(two_hop_candidates) > max_:
                # Random sampling or just slicing can be used. Slicing for speed.
                two_hop_candidates = two_hop_candidates[:max_]

            for idx, (rel, ent2) in enumerate(two_hop_candidates):
                self.connections_2hop[id_, idx, 0] = rel
                self.connections_2hop[id_, idx, 1] = ent2

        return None

    # --- META EXTRACTION with 2-hop inclusion ---
    ...
    def train(self):
        ...
        for data in train_generate(self.dataset, self.batch_size, self.train_few, self.symbol2id, self.ent2id, self.e1rel_e2):
            ...
            # --- ATOMIC FIX: Ensure evaluation happens at the end ---
    ...
    def eval(self, mode='dev', meta=False):
        ...
        # --- ATOMIC FIX: Point to dev_tasks.json ---
